...
|--------|----------|-------------|----------|-------|
| `POST` | `/auth/register` | `{ username: string, password: string }` | `{ id, username, token }` | Hash password di server (bcrypt) |
| `POST` | `/auth/login` | `{ username: string, password: string }` | `{ id, username, token }` | Return JWT token untuk auth |
| `GET` | `/auth/me` | — (Header: `Authorization: Bearer <token>`) | `{ id, username }` | Verifikasi token valid |
...
| `POST` | `/rooms/:code/join` | — (Auth header) | `{ room_id, code, players: [...] }` | Max capacity? TBD. Return list player. |
| `GET` | `/rooms/:code` | — | `{ id, code, host_id, status, players, created_at }` | Info room + daftar player |
| `POST` | `/rooms/:code/start` | — (Auth header, must be host) | `{ status: "playing", started_at }` | Hanya host. Trigger game start. |

#### Snippets

| Method | Endpoint | Request | Response | Notes |
|--------|----------|---------|----------|-------|
| `GET` | `/snippets/random` | — | `{ id, title, context, code, max_score }` | Random snippet. Bug descriptions NOT returned ke client (cegah bocor jawaban). |
...
```
Client → Server:
  game:join         { roomCode }                         → bergabung ke room
  game:leave        { roomCode }                         → keluar dari room
  game:submit       { roomCode, snippetId, answer }      → submit jawaban (string)

Server → Client:
  game:player-joined   { username, playerCount }            → broadcast ke room
  game:player-left     { username, playerCount }            → broadcast ke room
  game:started         { totalRounds, roundDuration }       → game dimulai, broadcast
  game:round-start     { round, snippet: {id,title,context,code}, timeLimit }  → soal & timer
  game:round-end       { round }                            → waktu habis
  game:score           { username, score, maxScore, feedback, bugsFound }  → hasil scoring per player
  game:leaderboard     [{ username, totalScore }, ...]      → leaderboard terupdate
  game:over            { winner, finalLeaderboard }         → game selesai
```

### API Contract — Strategic Notes
...
- 🏫 Mandatory requirement: "Wajib implementasi Real Time Communication"

**Kenapa bug descriptions nggak dikirim ke client?**
- `/snippets/random` hanya return `context` + `code`, bukan `bug_descriptions`
- Jawaban (kunci) disimpan di server → dikirim ke AI bersama jawaban user
...
Browser (React SPA)
    │
    ├── REST (fetch) ────── Express Server ───── PostgreSQL/JSON
    │                            │
    └── WebSocket ─── Socket.IO ─┤
                                 │
                            DeepSeek API
...
| HTTP client | fetch vs axios | fetch: built-in, no deps. axios: interceptor enak buat auth header, tapi tambahan dependency. | fetch + wrapper sederhana |
...
| Database | PostgreSQL vs SQLite vs JSON file | PG: production-grade, tapi perlu setup. SQLite: zero setup, cukup buat 3 user. JSON file: paling simpel, tapi no query. | SQLite (MVP) → PostgreSQL (kalau ada waktu) |
...
| **Git Management** | Althaff | GitHub Organization setup, branch protection, PR review gate, commit quality check (🏫 4 commit/hari) | All |
...
| **Deployment** | Adri | Client deploy (Vercel), server deploy (Railway/Render), environment variables, demo readiness | Resky |
...
- PR harus di-review oleh PJ area yang disentuh sebelum merge.
- Adri sebagai compliance gatekeeper: sebelum demo, checklist semua 🏫 mandatory requirement.
...
| 5 | **Deploy target** | Open | Client wajib deploy. Server opsional. Opsi: Vercel (client), Railway/Render (server). |
...
| 🎯 **Demo** | Presentasi ke instructor | **Kamis 09:00** |
...
| E3.5 | Deploy client ke Vercel | Adri | E2.8 | URL publik bisa diakses |
...
| Waktu | Althaff | Resky | Adri |
|-------|---------|-------|------|
| **Rabu siang** | E1.1, E1.4, E1.5, E1.6 (server + DB + auth) | E1.2, E1.3, E1.7, E1.8 (mock + auth UI + lobby) | E3.1 (bikin soal), E3.5 (setup Vercel) |
