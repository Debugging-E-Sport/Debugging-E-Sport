...
|--------|----------|-------------|----------|-------|
| `POST` | `/auth/register` | `{ username, password }` | `{ id, username, token }` | Hash password (bcrypt) |
| `POST` | `/auth/login` | `{ username, password }` | `{ id, username, token }` | Return JWT |
...
| Database | PostgreSQL vs SQLite vs JSON file | PG: production-grade, tapi perlu setup. SQLite: zero setup, cukup buat 3 user. JSON file: paling simpel, tapi no query. | SQLite (MVP) |
...
| **Adri (PM)** | Unblock Althaff/Resky, mulai bikin snippets, setup Vercel project, siapkan promptfoo. |
...
| Nggak sempat deploy server | Medium | 🏫 Server deploy opsional. Client wajib deploy ke Vercel. Server bisa jalan dari laptop via Tailscale. |
