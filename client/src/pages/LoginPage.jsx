...
export default function LoginPage() {
  const { activeTab, switchTab } = useAuthContext();
  const codeRainRef = useRef(null);

  useEffect(() => {
    const c = codeRainRef.current;
    if (!c) return;
...
Powered by Resky Altaf Andri`}
        ...
        <div className="space-y-5">
          <div className="flex flex-wrap gap-3">
            ...
            <div className="bg-arena-panel border border-arena-border rounded-full px-4 py-2 font-mono text-xs flex items-center gap-2">
              <i className="fa-solid fa-bug text-arena-green" /> 15,000+ Bugs Squashed
            </div>
            <div className="bg-arena-panel border border-arena-border rounded-full px-4 py-2 font-mono text-xs flex items-center gap-2">
              <i className="fa-solid fa-robot text-arena-green" /> Real-time AI Scoring
            </div>
          </div>
        ...
        <div className="w-full max-w-md bg-arena-panel border border-arena-border rounded-xl shadow-2xl p-8">
          ...
          <div className="flex border-b border-arena-border mb-7">
            <button
              ...
              className={`flex-1 pb-3 font-mono text-sm font-semibold transition-colors hover:text-arena-green ${
                activeTab === 'login' ? 'tab-active' : 'text-arena-muted'
              }`}
            >
              Login
            </button>
            <button
              ...
              className={`flex-1 pb-3 font-mono text-sm font-semibold transition-colors hover:text-arena-green ${
                activeTab === 'register' ? 'tab-active' : 'text-arena-muted'
              }`}
            >
              Register
            </button>
